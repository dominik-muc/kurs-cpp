...
double Add::evaluate() const { return e1->evaluate() + e2->evaluate(); }

Sub::Sub(Expression* e1, Expression* e2) : Binary(e1, e2) {}

std::string Sub::to_string() const {
    std::string es2 = (this->e2)->to_string();
    if (typeid(*e2) == typeid(Add) || typeid(*e2) == typeid(Sub))
        es2 = "(" + es2 + ")";
    return (this->e1)->to_string() + " - " + es2;
}

double Sub::evaluate() const { return e1->evaluate() - e2->evaluate(); }
...
double Multiply::evaluate() const { return e1->evaluate() * e2->evaluate(); }
...
std::string Divide::to_string() const {
    ...
    std::string es2 = (this->e2)->to_string();
    if (typeid(*e1) == typeid(Add) || typeid(*e1) == typeid(Sub) ||
        typeid(*e1) == typeid(Multiply))
        es1 = "(" + es1 + ")";
    if (typeid(*e2) == typeid(Add) || typeid(*e2) == typeid(Sub) ||
        typeid(*e1) == typeid(Multiply) || typeid(*e1) == typeid(Divide))
        es2 = "(" + es2 + ")";
    return es1 + " / " + es2;
}

double Divide::evaluate() const { return e1->evaluate() / e2->evaluate(); }
...
std::string Modulo::to_string() const {
    ...
    if (typeid(*e1) == typeid(Add) || typeid(*e1) == typeid(Sub) ||
        typeid(*e1) == typeid(Multiply) || typeid(*e1) == typeid(Divide) ||
        typeid(*e1) == typeid(Log) || typeid(*e1) == typeid(Modulo)) {
        es1 = "(" + es1 + ")";
    }
    if (typeid(*e2) == typeid(Add) || typeid(*e2) == typeid(Sub) ||
        typeid(*e2) == typeid(Multiply) || typeid(*e2) == typeid(Divide) ||
        typeid(*e2) == typeid(Log) || typeid(*e2) == typeid(Modulo)) {
        es2 = "(" + es2 + ")";
    }
    return es1 + " % " + es2;
}
...
std::string Power::to_string() const {
    ...
    if (typeid(*e1) == typeid(Add) || typeid(*e1) == typeid(Sub) ||
        typeid(*e1) == typeid(Multiply) || typeid(*e1) == typeid(Divide) ||
        typeid(*e1) == typeid(Power) || typeid(*e1) == typeid(Log) ||
        typeid(*e1) == typeid(Modulo)) {
        es1 = "(" + es1 + ")";
    }
    if (typeid(*e2) == typeid(Add) || typeid(*e2) == typeid(Sub) ||
        typeid(*e2) == typeid(Multiply) || typeid(*e2) == typeid(Divide) ||
        typeid(*e2) == typeid(Power) || typeid(*e2) == typeid(Log) ||
        typeid(*e2) == typeid(Modulo)) {
        es2 = "(" + es2 + ")";
    }
    return es1 + " ^ " + es2;
}
